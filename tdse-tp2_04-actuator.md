...
*   **Tarea 1 (Módulo System):** Corresponde a la ejecución de `task_system_update`. Es el núcleo lógico del programa. Esta tarea consume los eventos generados por los sensores y hace evolucionar la máquina de estados central (System Statechart), decidiendo en base a su estado qué señales enviar al actuador.
*   **Tarea 2 (Módulo Actuator):** Corresponde a la ejecución de `task_actuator_update`. Su función es consumir los eventos despachados por el sistema y traducirlos en acciones físicas sobre los periféricos de salida, controlando el encendido, apagado o parpadeo de los LEDs de la barrera.
